...
class DeviceManager:
    ...
    def register_device(self, device_name: str, device: GenericDevice):
        ...
        if device.responsive:
            ...
            print(
                f"{base.format(color=bcolors.WARNING)}, but it was unresponsive (possibly under kernel control?){bcolors.ENDC}"
            )

    def forget_device(self, device_name: str):
        self.devices.pop(device_name)
        print(
            f"{bcolors.WARNING}Removed device [{device_name}] from device manager.{bcolors.ENDC}"
        )
    ...
    def print_status(self):
        ...
        table = PrettyTable(
            [
                f"{bcolors.HEADER}STAT{bcolors.ENDC}",
                f"{NAME_COLOR}NAME{bcolors.ENDC}",
                f"{ADDRESS_COLOR}ADDR{bcolors.ENDC}",
            ]
        )
        for name, device in self.devices.items():
            if device.responsive:
                ...
                response_str = "XX"
            row = [
                f"{color}[{response_str}]{bcolors.ENDC}",
                f"{NAME_COLOR}{name}{bcolors.ENDC}",
                f"{ADDRESS_COLOR}{hex(device.address)}{bcolors.ENDC}",
            ]
            table.add_row(row)
...
def _test_bcolors():
    ...
    print(
        f"{bcolors.ENDC} This is the ENDC color" + f"{bcolors.FAIL} with the FAIL color"
    )
